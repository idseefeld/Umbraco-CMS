...
internal sealed class SetDateDefaultsToUtcNowTests : UmbracoIntegrationTest
{
    ...
    private static bool IsPostgreSql()
    {
        if (BaseTestDatabase.IsPostgreSql() is false)
        {
            return false;
        }

        Assert.Ignore("Named default constraints are a SQL Server concept, so the migration is a no-op on PostgreSQL.");
        return true;
    }
