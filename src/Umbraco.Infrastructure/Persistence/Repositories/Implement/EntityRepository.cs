...
internal sealed class EntityRepository : RepositoryBase, IEntityRepositoryExtended
{
        ...
        Action<Sql<ISqlContext>>? sqlCustomization = null)
    {
        ...
        var quotedColumnName = SqlSyntax.GetQuotedColumnName("NodeId");
        sql = ordering.Direction == Direction.Ascending ? sql.OrderBy(quotedColumnName) : sql.OrderByDescending(quotedColumnName);
        ...
        Page<GenericContentEntityDto>? page;
        try
        {
            page = Database.Page<GenericContentEntityDto>(pageIndexToFetch, pageSize, sql);
        }
        catch (Exception)
        {
            throw;
